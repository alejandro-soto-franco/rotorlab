//! LinePipeline: an instanced screen-space-thickness rectangle pipeline
//! that renders each line segment as an antialiased rectangle whose width
//! (perpendicular to the line) is constant in screen-space pixels.
//!
//! Each instance is one [`LineInstance`] (two world-space endpoints, RGBA
//! color, screen-pixel thickness). The vertex shader projects both
//! endpoints to clip space, computes a screen-space perpendicular normal,
//! and expands the segment into a four-vertex `TRIANGLE_STRIP` quad whose
//! corners are emitted from `gl_VertexIndex` (no per-vertex buffer is
//! bound; the only buffer is the instance buffer at binding 0).
//!
//! Push constants carry the camera `view_proj` matrix and the framebuffer
//! `viewport` size in pixels (the vertex shader needs the viewport to
//! convert pixel thickness into clip-space offsets).
//!
//! Plan 3 Task 6. The PNG hash-compare baseline test the spec mentions
//! (a 45-degree line rendering with the expected screen-space thickness)
//! is deferred to Plan 3 Task 13 (visual regression baselines); the smoke
...
/// One instance of a line segment: two homogeneous world-space endpoints,
/// linear RGBA color, and a thickness in screen-space pixels.
///
/// Thickness is in **screen-space pixels** (matches the `radius` semantic
/// on [`PointInstance`](crate::render::pipelines::PointInstance)): a line
/// with `thickness = 2.0` always covers a 2-pixel-wide rectangle on
/// screen, regardless of camera distance or projection. This is the
/// natural choice for explainer videos where reading geometry matters
/// more than reading distances.
///
/// `_pad` brings the struct to 64 bytes (a multiple of 16 bytes, the
/// typical instance-buffer alignment requirement on desktop GPUs).
/// Endpoints use `w = 1.0` for finite points; `w = 0.0` would denote a
/// direction and is not currently used.
#[repr(C)]
#[derive(Copy, Clone, Pod, Zeroable, Debug)]
pub struct LineInstance {
    /// Endpoint A in world space (homogeneous, `w = 1.0` for finite points).
    pub a: [f32; 4],
    /// Endpoint B in world space (homogeneous, `w = 1.0` for finite points).
    pub b: [f32; 4],
    /// Linear-space RGBA color. Alpha is multiplied by the rectangle's
    /// edge antialiasing alpha in the fragment shader.
    pub color: [f32; 4],
    /// Line thickness in **screen-space pixels** (constant on screen).
    pub thickness: f32,
    /// Padding to bring the struct up to 64 bytes (16-byte aligned).
    pub _pad: [f32; 3],
}

/// Push-constant block for the line pipeline.
///
/// Total size is `64 + 8 + 8 = 80` bytes, well under Vulkan's
/// minimum-required push-constant size of 128 bytes. Same shape as
/// [`PointPushConstants`](crate::render::pipelines::PointPushConstants)
/// without the per-frame `time` field (lines do not need it for Plan 3).
#[repr(C)]
#[derive(Copy, Clone, Pod, Zeroable, Debug)]
pub struct LinePushConstants {
    ...
    /// screen-pixel thickness into clip-space offsets.
    pub viewport: [f32; 2],
    /// Trailing pad so the block is 16-byte aligned (and explicit).
    pub _pad: [f32; 2],
}

const LINE_VERT_SPV: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/line.vert.spv"));
const LINE_FRAG_SPV: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/line.frag.spv"));

/// Instanced screen-space-thickness rectangle pipeline.
...
impl LinePipeline {
    ...
    /// command buffer via `cmd_set_viewport` / `cmd_set_scissor`. This
    /// lets the same pipeline handle resolution changes in Plan 5+
    /// without a rebuild.
    pub fn new(device: Arc<Device>, render_pass: &RenderPass) -> Result<Self, RenderError> {
        ...
        let vert_module = create_shader_module(&device, LINE_VERT_SPV)?;
        let frag_module = create_shader_module(&device, LINE_FRAG_SPV)?;
        ...
        // Binding 0: one LineInstance per instance (stride = 64 bytes).
        // No per-vertex buffer: the quad corners are emitted from
        // gl_VertexIndex inside the shader.
        let bindings = [vk::VertexInputBindingDescription::default()
            .binding(0)
            .stride(mem::size_of::<LineInstance>() as u32)
            .input_rate(vk::VertexInputRate::INSTANCE)];
        // Attribute 0: a         (R32G32B32A32_SFLOAT, offset  0).
        // Attribute 1: b         (R32G32B32A32_SFLOAT, offset 16).
        // Attribute 2: color     (R32G32B32A32_SFLOAT, offset 32).
        // Attribute 3: thickness (R32_SFLOAT,          offset 48).
        let attrs = [
        ...
        // Triangle/Point) so the rectangle's antialiased edge composites
        ...
        // full LinePushConstants block, visible to vertex + fragment.
        let push_range = vk::PushConstantRange::default()
            .stage_flags(vk::ShaderStageFlags::VERTEX | vk::ShaderStageFlags::FRAGMENT)
            .offset(0)
            .size(mem::size_of::<LinePushConstants>() as u32);
        ...
        // TrianglePipeline / PointPipeline
        // (Result<Vec<P>, (Vec<P>, vk::Result)>).
...
/// A host-visible instance buffer for line instances.
///
...
impl LineInstanceBuffer {
    ...
    pub fn new(device: Arc<Device>, capacity: u32) -> Result<Self, RenderError> {
        ...
        let memory_type_index =
            find_memory_type(&mem_props, requirements.memory_type_bits, required_flags)
                .ok_or_else(|| {
                    RenderError::OutOfMemory(
                        "no HOST_VISIBLE | HOST_COHERENT memory type for line instance buffer"
                            .into(),
                    )
                })?;
    ...
    pub fn upload(&mut self, instances: &[LineInstance]) -> Result<(), RenderError> {
        if instances.len() as u64 > self.capacity as u64 {
            return Err(RenderError::OutOfMemory(format!(
                "line instance upload of {} exceeds capacity {}",
                instances.len(),
                self.capacity
            )));
        }
...
impl Drop for LineInstanceBuffer {
    fn drop(&mut self) {
...
/// `triangle.rs`, `point.rs`, and `render_target.rs`; dedupe is a future
/// refactor).
fn find_memory_type(
...
#[cfg(test)]
mod tests {
    // The PNG hash-compare baseline test the spec mentions (a 45-degree
    // line rendering with the expected screen-space thickness) is deferred
    // to Plan 3 Task 13 ("Visual regression baselines"). The smoke tests
    // here prove the pipeline + instance buffer + push constants compile
    // and bind without Vulkan validation errors, which is most of the
    // work; Task 13 will add pixel-level validation.

    use super::*;
    use crate::render::{Device, Instance, RenderPass};

    #[test]
    fn line_push_constants_size_is_eighty() {
        // 64 (view_proj) + 8 (viewport) + 8 (_pad) = 80.
        assert_eq!(std::mem::size_of::<LinePushConstants>(), 80);
    }

    #[test]
    fn line_instance_size_is_sixty_four() {
        // 16 (a) + 16 (b) + 16 (color) + 4 (thickness) + 12 (_pad) = 64.
        assert_eq!(std::mem::size_of::<LineInstance>(), 64);
    }

    #[test]
    fn create_line_pipeline_smoke() {
        let Ok(i) = Instance::new() else { return };
        let Ok(d) = Device::new(i) else { return };
        let pass = RenderPass::new(d.clone()).expect("render pass");
        let p = LinePipeline::new(d, &pass).expect("line pipeline");
        drop(p);
    }

    #[test]
    fn create_line_instance_buffer_smoke() {
        let Ok(i) = Instance::new() else { return };
        let Ok(d) = Device::new(i) else { return };
        let buf = LineInstanceBuffer::new(d, 16).expect("instance buffer");
        drop(buf);
    }

    #[test]
    fn line_instance_buffer_upload_within_capacity() {
        let Ok(i) = Instance::new() else { return };
        let Ok(d) = Device::new(i) else { return };
        let mut buf = LineInstanceBuffer::new(d, 4).expect("buffer");
        let instances = [
            LineInstance {
                a: [0.0, 0.0, 0.0, 1.0],
                b: [1.0, 0.0, 0.0, 1.0],
    ...
    #[test]
    fn line_instance_buffer_upload_overflow_errors() {
        let Ok(i) = Instance::new() else { return };
        let Ok(d) = Device::new(i) else { return };
        let mut buf = LineInstanceBuffer::new(d, 1).expect("buffer");
        let instances = [
            LineInstance {
                a: [0.0; 4],
                b: [0.0; 4],
